...
- **Repository:** Migz93/ha-virtual_gas_meter

**Key directories:**

- `custom_components/virtual_gas_meter/` - Main integration code
...
- Integration logs: `custom_components.virtual_gas_meter: debug` in `config/configuration.yaml`
...
- Use the domain `virtual_gas_meter` for all DOMAIN references
- Prefix all integration-specific classes with `VirtualGasMeter`
- Use "Virtual Gas Meter" as the display title
...
- Inherit from platform base + `VirtualGasMeterEntity`
...
- `tests/` mirrors `custom_components/virtual_gas_meter/` structure
