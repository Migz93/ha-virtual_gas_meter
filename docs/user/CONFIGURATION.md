...
This document describes all configuration options and settings available in the Virtual Gas Meter custom integration.
...
### `virtual_gas_meter.example_service`
...
```yaml
service: virtual_gas_meter.example_service
target:
  ...
  - alias: "Call service at sunset"
    trigger:
      - trigger: sun
        event: sunset
    action:
      - action: virtual_gas_meter.example_service
        target:
          entity_id: switch.device_name_switch
        data:
          parameter: "mode"
...
- [GitHub Issues](https://github.com/Migz93/ha-virtual_gas_meter/issues) - Report problems
