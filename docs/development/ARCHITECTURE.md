...
This document describes the technical architecture of the Virtual Gas Meter custom component for Home Assistant.

## Directory Structure

```text
custom_components/virtual_gas_meter/
...
│   ├── __init__.py          # Exports VirtualGasMeterDataUpdateCoordinator
...
│   ├── __init__.py          # Exports VirtualGasMeterEntity
...
- `base.py` - Main coordinator class (`VirtualGasMeterDataUpdateCoordinator`)
...
**Key class:** `VirtualGasMeterDataUpdateCoordinator` (exported from `coordinator/__init__.py`)
...
**Key class:** `VirtualGasMeterApiClient`
...
- `VirtualGasMeterConfigFlowHandler` (main flow)
- `VirtualGasMeterOptionsFlow` (options)
...
**Key class:** `VirtualGasMeterEntity` (in `entity/base.py`)
...
2. `VirtualGasMeterEntity` for common functionality
...
1. Create directory: `custom_components/virtual_gas_meter/<platform>/`
2. Implement `__init__.py` with `async_setup_entry()`
3. Create entity classes inheriting from platform base + `VirtualGasMeterEntity`
