...
Every Monday at 07:00 UTC, the workflow checks whether the upstream blueprint (`Migz93/ha-virtual_gas_meter`) has new commits. If it does, it opens a pull request with the diff against your repository.
...
That's it. No workflow runs, no PRs, no noise. You can still pull upstream changes manually at any time by comparing your repository against `Migz93/ha-virtual_gas_meter`.
